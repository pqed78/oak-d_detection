#!/usr/bin/env python3
"""
calculate distance between the camera and object in color camera
"""
...
config.depthThresholds.lowerThreshold = 100 #minimum distance in mm
config.depthThresholds.upperThreshold = 10000 #maximum distance in mm
...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        In case depthFrame is aligned with color camera, the shape of images are the same except the number of color channel)
        """
        ...
        for depthData in spatialData:
            ...
            xmin = int(roi.topLeft().x) # unit in pixel
            ...
            cv2.rectangle(videoFrame, (xmin, ymin), (xmax, ymax), color, 1)
            cv2.putText(videoFrame, f"X: {int(depthData.spatialCoordinates.x)} mm", (xmin + 10, ymin + 20), fontType, 0.5, color)
            cv2.putText(videoFrame, f"Y: {int(depthData.spatialCoordinates.y)} mm", (xmin + 10, ymin + 35), fontType, 0.5, color)
            cv2.putText(videoFrame, f"Z: {int(depthData.spatialCoordinates.z)} mm", (xmin + 10, ymin + 50), fontType, 0.5, color)
        # Show the frame
        cv2.imshow("depth", depthFrameColor)
        cv2.imshow("color", videoFrame)
